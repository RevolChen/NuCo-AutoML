...
if __name__ == '__main__':
    ...
    # 5. [前置] 执行 OOM 预测试
    print("\n2. 开始 OOM 预测试机制 (动态调整 batch_size 搜索空间)...")
    while True:
        max_bs = max(BATCH_SIZE_LIST)
        ...
        if is_success:
            print("    -> 测试通过！当前安全 batch_size 空间:", BATCH_SIZE_LIST)
            break
        else:
            print(f"    -> [OOM] batch_size={max_bs} 连续两次触发显存溢出！剔除 {max_bs} 并添加更小的值...")
            if max_bs in BATCH_SIZE_LIST:
                BATCH_SIZE_LIST.remove(max_bs)
            if len(BATCH_SIZE_LIST) == 0:
                BATCH_SIZE_LIST.append(max(1, max_bs // 2))
            else:
                next_min = max(1, min(BATCH_SIZE_LIST) // 2)
                if next_min not in BATCH_SIZE_LIST:
                    BATCH_SIZE_LIST.append(next_min)
            BATCH_SIZE_LIST.sort()

    print("    -> OOM 测试结束。最终安全的 batch_size 搜索空间为:", BATCH_SIZE_LIST)

    if best_init_params.get('batch_size') not in BATCH_SIZE_LIST:
        best_init_params['batch_size'] = max(BATCH_SIZE_LIST)
