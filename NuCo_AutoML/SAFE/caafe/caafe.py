...
def generate_features(
    ...
    def generate_code(messages):
        ...
        #     spark_app_id='febde2c1',
        #     spark_api_key='d1bc5b20be9b32650e33601d4f626ab4',
        #     spark_api_secret='MzY4ODYxMGE5NGQ3NmQyOTkxOGMzMTAy',
        #     spark_llm_domain='4.0Ultra',
        #     streaming=False,
        #     temperature=0.5,  # 类似 OpenAI 的 temperature
        #     max_tokens=500,  # 类似 OpenAI 的 max_tokens
        # )
        ...
        client = OpenAI(
            api_key="sk-8FnHBq5zuIyDAKI2lM6RAhJ0TyPMM70MqW5dSj2cgLjK8wlT",
    ...
    ###################add#################
    # messages = [
    #     ChatMessage(role="system", content="You are an expert datascientist assistant solving Kaggle problems. You answer only by generating code. Answer as concisely as possible."),
    ...
    while i < n_iter:
        ...
        if e is not None:
            ...
            #                 content=code),
            #     ChatMessage(role="user", content=f"""Code execution failed with error: {type(e)} {e}.\n Code: ```python{code}```\n Generate next feature (fixing error?):
            #                                       ```python
            #                                       """)
            # ]
            ###################add#######################
        ...
        if len(code) > 10:
            ...
            #     ChatMessage(role="assistant",
            #                 content=code),
            #     ChatMessage(role="user", content=f"""Performance after adding feature ROC {np.nanmean(rocs):.3f}, ACC {np.nanmean(accs):.3f}. {add_feature_sentence}
            #                                       Next codeblock:
            #                                       """)
            # ]
            ####################add#########################
