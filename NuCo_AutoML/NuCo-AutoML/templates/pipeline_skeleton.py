...
CONFIG = {
    ...
    "TASK_TYPE": "{task_type}",
    "MODALITY_MAP": {modality_map_json},
    "MODEL_SELECTION": {model_config_json},
    "SEED": {seed}
}
...
if __name__ == '__main__':
    ...
    # 全局变量记录确定的 batch_size 搜索空间
    BATCH_SIZE_LIST = {batch_size_list}
    BATCH_SIZE_LIST = sorted(list(set(BATCH_SIZE_LIST)))
    
    # 全局变量记录触发OOM的最小 batch_size
    GLOBAL_MAX_BATCH_SIZE = float('inf')

    # 6. 定义超参优化目标函数
    def objective(trial, is_test=False, test_batch_size=None):
        ...
        if is_test and test_batch_size is not None:
            params['batch_size'] = test_batch_size
        else:
            params['batch_size'] = trial.suggest_categorical('batch_size', BATCH_SIZE_LIST)

        # OOM 防护拦截：如果当前 batch_size 已经大于等于曾经爆显存的 batch_size，直接剪枝
        ...
        try:
            ...
            if "out of memory" in str(e).lower():
                ...
                print(f"[OOM] Trial {trial.number} 显存溢出！已触发保护机制，后续 Trial 的 batch_size 必须严格小于 {GLOBAL_MAX_BATCH_SIZE}。")
                gc.collect()
    ...
    # 7. 执行 HPO
    if args.trials > 0:
        print("\n2. 开始 OOM 压力测试以确定最大可用 batch_size...")
        best_init_params = {best_init_params}
        
        while len(BATCH_SIZE_LIST) > 0:
            test_bs = BATCH_SIZE_LIST[-1]
            print(f"\n[*] 测试 batch_size = {test_bs}")
            
            # 使用 Optuna 跑一个 trial 进行测试
            ...
            study_test.enqueue_trial(test_params)
            
            def test_objective(trial):
                # 仅跑1个 Epoch 快速测试 OOM
                global MAX_EPOCHS
                old_max_epochs = MAX_EPOCHS
                MAX_EPOCHS = 1
                try:
                    score = objective(trial, is_test=True, test_batch_size=test_bs)
                finally:
            ...
            study_test.optimize(test_objective, n_trials=1)
            optuna.logging.set_verbosity(optuna.logging.INFO)
            
            if study_test.best_value == -99999.0:
                print(f"[!] batch_size = {test_bs} 测试失败 (OOM)，调整搜索空间...")
                BATCH_SIZE_LIST.pop()
                if len(BATCH_SIZE_LIST) > 0:
                    smallest = BATCH_SIZE_LIST[0]
                    new_smallest = max(2, smallest // 2)
                    if new_smallest not in BATCH_SIZE_LIST:
                        BATCH_SIZE_LIST.insert(0, new_smallest)
                        BATCH_SIZE_LIST = sorted(BATCH_SIZE_LIST)
                else:
                    BATCH_SIZE_LIST = [max(2, test_bs // 2)]
                print(f"[*] 新的 batch_size 候选: {BATCH_SIZE_LIST}")
            else:
                print(f"[*] batch_size = {test_bs} 测试通过！")
                break
                
        print(f"\n3. 开始正式超参优化 (Trials={args.trials}), BATCH_SIZE_LIST={BATCH_SIZE_LIST} ...")
        ...
        best_init_params['batch_size'] = BATCH_SIZE_LIST[-1] # 使用测试通过的最大 batch_size
        print("[*] 注入 LLM 推荐的初始参数:", best_init_params)
        study.enqueue_trial(best_init_params)
        
        study.optimize(objective, n_trials=args.trials)
        best_params = study.best_params
        best_epoch = study.best_trial.user_attrs.get("best_epoch", 1)
